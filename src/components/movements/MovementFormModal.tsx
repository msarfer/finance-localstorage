...
export function MovementFormModal({
  ...
  const [toAccountId, setToAccountId] = useState<string>(
    initial?.toAccountId ?? onlineAccounts[1]?.id ?? onlineAccounts[0]?.id ?? '',
  )
  ...
  const [cashflowIds, setCashflowIds] = useState<{ bank: string; wallet: string }>(() => {
    const from = accountById.get(initial?.fromAccountId ?? '')
    const to = accountById.get(initial?.toAccountId ?? '')
    return {
      bank: from?.kind === 'online' ? from.id : to?.kind === 'online' ? to.id : onlineAccounts[0]?.id ?? '',
      wallet: from?.kind === 'cash' ? from.id : to?.kind === 'cash' ? to.id : cashAccounts[0]?.id ?? '',
    }
  })
  ...
  const cashflowFrom = direction === 'toCash' ? cashflowIds.bank : cashflowIds.wallet
  const cashflowTo = direction === 'toCash' ? cashflowIds.wallet : cashflowIds.bank

  const cashAffected =
    type === 'cashflow' ? true : type === 'transfer' ? false : selectedAccount?.kind === 'cash'
  ...
  const submit = () => {
    ...
    if (type === 'transfer') {
      ...
      if (!cashflowIds.bank || !cashflowIds.wallet) {
        setError('Necesitas una cuenta online y una de efectivo para esta operación')
        return
      }
      if (direction === 'toBank') {
        if (walletAccount && !hasEnoughCash(walletAccount.cash, cashBreakdown)) {
          setError(`No hay suficientes billetes/monedas en "${walletAccount.name}" para ingresar`)
          return
        }
      }
      const payload = {
        ...
        {type === 'cashflow' && (
          <>
            <div className="flex gap-2">
              {dirBtn('toCash', '🏧 Sacar (banco → efectivo)')}
              {dirBtn('toBank', '🏦 Ingresar (efectivo → banco)')}
            </div>
            <div className="grid grid-cols-2 gap-3">
              <Field label="Cuenta bancaria (online)">
                <select className={inputCls} value={cashflowIds.bank} onChange={(e) => setCashflowIds((p) => ({ ...p, bank: e.target.value }))}>
                  {onlineAccounts.map((a) => (
                    <option key={a.id} value={a.id}>{a.name}</option>
                  ))}
                </select>
              ...
              <Field label="Cartera (efectivo)">
                <select className={inputCls} value={cashflowIds.wallet} onChange={(e) => setCashflowIds((p) => ({ ...p, wallet: e.target.value }))}>
                  {cashAccounts.map((a) => (
                    <option key={a.id} value={a.id}>{a.name}</option>
                  ))}
                </select>
