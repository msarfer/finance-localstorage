...
export function Layout({
  ...
  const total = accounts.reduce(
    (sum, a) => sum + (a.kind === 'cash' ? computeCashTotal(a.cash ?? {}) : a.balance),
    0,
  )
  ...
  return (
    <div className="min-h-screen bg-slate-100 text-slate-900 dark:bg-slate-950 dark:text-slate-100">
      ...
      <footer className="mx-auto max-w-5xl px-4 pb-8 text-center text-xs text-slate-400 dark:text-slate-500">
        Tus datos se guardan solo en este navegador (localStorage). Usa Importar / Exportar para hacer copias de seguridad.
      </footer>
    </div>
  )
}