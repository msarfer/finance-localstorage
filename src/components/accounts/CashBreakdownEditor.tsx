...
import { computeCashTotal, countsFromValue, formatEUR, parseCentsInput } from '@/lib/money'
  ...
  return (
    <li className="flex items-center justify-between gap-3 rounded-lg border border-slate-200 bg-white px-3 py-2 dark:border-slate-700 dark:bg-slate-800">
      ...
      <div className="flex items-center gap-1.5">
        ...
        />
        <button type="button" onClick={() => bump(1)} aria-label={`Añadir ${noun} de ${label}`} className={btnCls}>
          <svg className="h-4 w-4" viewBox="0 0 20 20" fill="currentColor" aria-hidden="true">
  ...
  return (
    <div className="space-y-4">
      <div role="group" aria-labelledby={billsId}>
        <p id={billsId} className="mb-2 text-sm font-semibold text-slate-600 dark:text-slate-300">
          Billetes
        </p>
        <ul className="grid grid-cols-1 gap-2 sm:grid-cols-2">
          {BILLS.map((d) => (
            <DenominationRow key={d} label={BILL_LABELS[d]} denom={d} count={value[d] ?? 0} onChange={(n) => setCount(d, n)} />
          ))}
        </ul>
      </div>

      <div role="group" aria-labelledby={coinsId}>
        <p id={coinsId} className="mb-2 text-sm font-semibold text-slate-600 dark:text-slate-300">
          Monedas
        </p>
        <ul className="grid grid-cols-1 gap-2 sm:grid-cols-2">
          {COINS.map((d) => (
            <DenominationRow key={d} label={COIN_LABELS[d]} denom={d} count={value[d] ?? 0} onChange={(n) => setCount(d, n)} />
          ))}
        </ul>
      </div>
      ...
      <div className="rounded-lg border border-dashed border-slate-300 bg-slate-50 p-3 dark:border-slate-700 dark:bg-slate-800/40">
        <label htmlFor={fillId} className="mb-1 block text-xs font-medium text-slate-500 dark:text-slate-400">
        ...
        <div className="flex gap-2">
          ...
          />
          <Button variant="secondary" onClick={applyFill} disabled={!fillValue.trim()} className="shrink-0">
            Rellenar
          </Button>
        </div>
        {fillError && <p className="mt-1 text-xs text-red-600 dark:text-red-400">{fillError}</p>}
      </div>
