...
function CategoryFormModal({
  ...
  const submit = () => {
    ...
    const payload = { name: name.trim(), emoji: emoji.trim() || undefined, color, type }
    ...
    <Modal
      ...
      footer={
        <>
          ...
          <Button onClick={submit}>{initial ? 'Guardar cambios' : 'Crear categoría'}</Button>
        </>
      }
    >
      <div className="space-y-4">
        <div className="grid grid-cols-[1fr_auto] gap-3">
          <Field label="Nombre">
            <input className={inputCls} value={name} onChange={(e) => setName(e.target.value)} placeholder="Ej. Gimnasio" />
          </Field>
          <Field label="Emoji">
            <input className={`${inputCls} w-20 text-center`} value={emoji} onChange={(e) => setEmoji(e.target.value)} placeholder="🏋️" />
          </Field>
        </div>
        <Field label="Tipo">
          <select className={inputCls} value={type} onChange={(e) => setType(e.target.value as CategoryType)}>
          ...
          <div className="flex flex-wrap gap-2">
            {CATEGORY_COLORS.map((c) => (
              <button
                ...
                type="button"
                onClick={() => setColor(c)}
                className={`h-8 w-8 rounded-full transition ${color === c ? 'ring-2 ring-offset-2 ring-slate-400' : ''}`}
...
export function CategoriesView() {
  ...
  const typeLabel = (t: CategoryType) => (t === 'income' ? 'Ingreso' : t === 'expense' ? 'Gasto' : 'Ambos')
  ...
  return (
    <div className="space-y-4">
        ...
        <div className="grid gap-2 sm:grid-cols-2">
          {categories.map((c) => {
            ...
            return (
              ...
              >
                <span className="flex h-9 w-9 shrink-0 items-center justify-center rounded-full text-base" style={{ backgroundColor: `${c.color}22` }}>
                  {c.emoji ?? '🏷️'}
                </span>
                <div className="min-w-0 flex-1">
                  <p className="truncate text-sm font-medium">{c.name}</p>
                  <p className="text-xs text-slate-500 dark:text-slate-400">
                    {typeLabel(c.type)} · {used} movimiento{used === 1 ? '' : 's'}
                  </p>
                </div>
                <button
                  type="button"
      ...
      {formOpen && (
        <CategoryFormModal key={editing?.id ?? 'new'} initial={editing ?? undefined} onClose={() => setFormOpen(false)} />
      )}
